...
export const users = sqliteTable('users', {
  ...
  language: text('language'), // User's preferred language (null = use IP geolocation)
  avatar: text('avatar'),
...
export const collections = sqliteTable('collections', {
  ...
  isActive: integer('is_active', { mode: 'boolean' }).notNull().default(true),
  managed: integer('managed', { mode: 'boolean' }).notNull().default(false), // @deprecated Use codeManaged and fieldsEditable instead
  codeManaged: integer('code_managed', { mode: 'boolean' }).notNull().default(false), // Collection is defined in code
  fieldsEditable: integer('fields_editable', { mode: 'boolean' }).notNull().default(true), // Fields can be edited in UI
...
export const content = sqliteTable('content', {
  ...
  title: text('title'), // Optional - can be extracted from data field
  data: text('data', { mode: 'json' }).notNull(), // JSON content data
  status: text('status').notNull().default('draft'), // 'draft', 'published', 'archived'
  // Language field: Database-level default is 'en', but application logic should use
  // AI Translator plugin's defaultSourceLanguage setting (see admin-content.ts route).
  // This 'en' default is only a safety fallback for edge cases.
  language: text('language').notNull().default('en'),
  translationGroupId: text('translation_group_id'),
  translationSource: text('translation_source').notNull().default('manual'), // 'manual', 'ai'
...
export const activityLogs = sqliteTable('activity_logs', {
  ...
  resourceType: text('resource_type').notNull(), // 'content', 'media', 'users', 'settings'
  resourceId: text('resource_id'),
  details: text('details'), // JSON details about the change
  userId: text('user_id').references(() => users.id),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().$defaultFn(() => new Date()),
});
