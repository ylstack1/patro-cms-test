...
export function renderAPIReferencePage(data: APIReferencePageData, t: TranslateFn): string {
  ...
  const categoryInfo = {
    'Auth': {
      title: t('apiReference.auth'),
      description: t('apiReference.authDesc'),
      icon: '🔐'
    },
    'Content': {
      title: t('apiReference.content'),
      description: t('apiReference.contentDesc'),
      icon: '📝'
    },
    'Media': {
      title: t('apiReference.media'),
      description: t('apiReference.mediaDesc'),
      icon: '🖼️'
    },
    'Admin': {
      title: t('apiReference.admin'),
      description: t('apiReference.adminDesc'),
      icon: '⚙️'
    },
    'System': {
      title: t('apiReference.system'),
      description: t('apiReference.systemDesc'),
      icon: '🔧'
    }
  }
  ...
  const pageContent = `
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl/8 font-semibold text-zinc-950 dark:text-white sm:text-xl/8">${t('apiReference.title')}</h1>
          <p class="mt-2 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('apiReference.subtitle')}</p>
        </div>
      ...
      <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 px-6 py-5">
          <dt class="text-sm/6 font-medium text-zinc-500 dark:text-zinc-400">${t('apiReference.totalEndpoints')}</dt>
        ...
        <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 px-6 py-5">
          <dt class="text-sm/6 font-medium text-zinc-500 dark:text-zinc-400">${t('apiReference.publicEndpoints')}</dt>
          <dd class="mt-2 flex items-baseline gap-x-2">
        ...
        <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 px-6 py-5">
          <dt class="text-sm/6 font-medium text-zinc-500 dark:text-zinc-400">${t('apiReference.protectedEndpoints')}</dt>
          <dd class="mt-2 flex items-baseline gap-x-2">
        ...
        <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 px-6 py-5">
          <dt class="text-sm/6 font-medium text-zinc-500 dark:text-zinc-400">${t('apiReference.categories')}</dt>
          <dd class="mt-2 flex items-baseline gap-x-2">
      ...
      <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 overflow-hidden">
        <div class="px-6 py-5">
          <div class="flex flex-col sm:flex-row sm:items-end gap-4">
            <div class="flex-1">
              <label class="block text-sm/6 font-medium text-zinc-950 dark:text-white mb-2">${t('apiReference.search')}</label>
              <div class="relative">
            ...
            <div>
              <label class="block text-sm/6 font-medium text-zinc-950 dark:text-white mb-2">${t('apiReference.method')}</label>
              <div class="grid grid-cols-1">
                ...
                >
                  <option value="">${t('apiReference.allMethods')}</option>
            ...
            <div>
              <label class="block text-sm/6 font-medium text-zinc-950 dark:text-white mb-2">${t('apiReference.category')}</label>
              <div class="grid grid-cols-1">
                <select
                  id="category-filter"
                ...
                >
                  <option value="">${t('apiReference.allCategories')}</option>
      ...
      <div id="no-results" class="hidden rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 p-12 text-center">
        ...
        <h3 class="mt-4 text-base/7 font-semibold text-zinc-950 dark:text-white">${t('apiReference.noResults')}</h3>
        <p class="mt-2 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('apiReference.noResultsMessage')}</p>
      </div>
    </div>
      ...
      document.addEventListener('click', function(e) {
        ...
        if (endpoint && e.target.tagName === 'CODE') {
          ...
          navigator.clipboard.writeText(path).then(() => {
            ...
            notification.innerHTML = '<div class="flex items-center gap-x-2"><svg class="h-5 w-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p class="text-sm font-medium text-white">${t('apiReference.copied')}</p></div>';
  ...
  const layoutData: AdminLayoutCatalystData = {
    ...
    pageTitle: t('apiReference.title'),
