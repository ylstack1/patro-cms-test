...
import { renderTemplateStandalone } from '../../../../utils/template-renderer';
...
export class EmailTemplateRenderer {
  ...
  async renderTemplate(
    ...
    // Render subject line
    const subject = Effect.runSync(renderTemplateStandalone(template.subject, allVariables));
  ...
  private getSystemVariables(): EmailVariables {
    ...
    return {
      site: {
        name: 'PatroCMS',
        url: 'https://patro-ai.com',
        logo: 'https://demo.patro.io/images/favicon.ico',
      },
      date: {
  ...
  private async renderMarkdownToHtml(markdown: string, variables: EmailVariables): Promise<string> {
    ...
    const processedMarkdown = Effect.runSync(renderTemplateStandalone(markdown, variables));
  ...
  private renderEmailHtml(theme: EmailTheme, bodyHtml: string, variables: EmailVariables): string {
    ...
    return Effect.runSync(renderTemplateStandalone(theme.htmlTemplate, allVariables));
  }
  ...
  static getDummyVariables(): EmailVariables {
    return {
      ...
      // Content specific
      content: {
        title: 'Getting Started with PatroCMS',
        url: 'https://example.com/content/123',
        excerpt: 'Learn how to build amazing web applications with PatroCMS...',
      },
      ...
      replyToEmail: 'support@patro-ai.com',
    };
  }
}
