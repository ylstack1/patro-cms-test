...
test.describe('Plugin Management', () => {
  ...
  test('should show AI Translator core plugin in plugin list', async ({ page }) => {
    await page.goto(`${BASE_URL}/admin/plugins`)
    
    // Wait for plugins to be bootstrapped and listed
    await page.waitForTimeout(2000)
    
    const aiTranslatorCards = page.locator('.plugin-card:has-text("AI Translator")')
    const count = await aiTranslatorCards.count()
