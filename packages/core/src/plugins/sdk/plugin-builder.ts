 ...
 * Plugin Builder SDK
 * 
 * Provides a fluent API for building PatroCMS plugins
 */
...
END;
  ...
  /**
   * Create an Effect Schema for a model
   */
  static createSchema(fields: Array<{
    ...
    for (const field of fields) {
      ...
      if (field.optional) {
        schema = Schema.optional(schema as any) as any
      }

      if (field.validation) {
        // Apply additional validation
        if (field.type === 'string' && field.validation.min) {
          schema = Schema.String.pipe(Schema.minLength(field.validation.min))
        }
        if (field.type === 'string' && field.validation.max) {
          schema = Schema.String.pipe(Schema.maxLength(field.validation.max))
        }
