 ...
 *
 * Provides Quill editor integration for rich text editing in PatroCMS
 * https://quilljs.com/
 */
    ...
    <script>
      ...
      window.initializeQuillEditors = function() {
        ...
        if (typeof Quill === 'undefined') {
          console.warn('⚠️ [Quill] Quill is not loaded yet. Retrying...');
        ...
        containers.forEach((container, index) => {
          ...
          console.log('🔍 [Quill] editorDiv for container', index, ':', editorDiv);
          
          if (!editorDiv) {
            console.warn('⚠️ [Quill] No editorDiv found in container', index);
            return;
          }
          
          if (editorDiv.classList.contains('ql-container')) {
            console.log('ℹ️ [Quill] Editor already initialized for container', index);
            return;
          }

          const fieldId = container.getAttribute('data-field-id');
          console.log('🔍 [Quill] Field ID:', fieldId);
          
          const hiddenInput = document.getElementById(fieldId);
          console.log('🔍 [Quill] Hidden input:', hiddenInput, 'Value:', hiddenInput?.value);
          ...
          console.log('🔍 [Quill] Config - theme:', theme, 'toolbar:', toolbarPreset, 'height:', height);
          ...
          console.log('✅ [Quill] Quill instance created for container', index);

          // Set editor height
          const editorElement = editorDiv.querySelector('.ql-editor');
          if (editorElement) {
            editorElement.style.minHeight = height + 'px';
            console.log('✅ [Quill] Editor height set to', height + 'px');
          }

          // Sync content to hidden input on change
          quill.on('text-change', function() {
            const html = quill.root.innerHTML;
            console.log('📝 [Quill] Content changed, updating hidden input');
            if (hiddenInput) {
          ...
          console.log('✅ [Quill] Successfully initialized editor', index, '- READY TO EDIT');
          } catch (error) {
            console.error('❌ [Quill] Error initializing editor', index, ':', error);
            console.error('❌ [Quill] Error stack:', error.stack);
          }
        });
        console.log('✅ [Quill] Initialization complete -', containers.length, 'editors processed');
      };
...
export function createQuillEditorPlugin(): Plugin {
  const builder = PluginBuilder.create({
    name: 'quill-editor',
    version: '1.0.0',
    description: 'Quill rich text editor integration for PatroCMS'
  })
