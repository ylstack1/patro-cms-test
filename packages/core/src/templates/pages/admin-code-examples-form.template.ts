...
export function renderCodeExamplesForm(data: CodeExamplesFormData, t: TranslateFn): string {
  const { codeExample, isEdit, errors, message, messageType } = data
  const pageTitle = isEdit ? t('codeExamples.edit') : t('codeExamples.create')
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p class="mt-2 text-sm text-gray-300">
            ${isEdit ? t('codeExamples.messages.updateError') : t('codeExamples.messages.createError')}
          </p>
        </div>
      ...
      <div class="backdrop-blur-xl bg-white/10 rounded-xl border border-white/20 shadow-2xl">
        <form ${isEdit ? `hx-put="/admin/code-examples/${codeExample?.id}"` : 'hx-post="/admin/code-examples"'}
          ...
          <div>
            <h2 class="text-lg font-medium text-white mb-4">${t('codeExamples.form.basicInfo')}</h2>

            <!-- Title -->
            <div class="mb-4">
              <label for="title" class="block text-sm font-medium text-white">
                ${t('codeExamples.snippetTitle')} <span class="text-red-400">*</span>
              </label>
              <div class="mt-1">
                <input type="text"
                       name="title"
                       ...
                       placeholder="${t('codeExamples.form.titlePlaceholder')}">
              </div>
              ${errors?.title ? `
                <div class="mt-1">
            ...
            <!-- Description -->
            <div class="mb-4">
              <label for="description" class="block text-sm font-medium text-white">${t('codeExamples.description')}</label>
              <div class="mt-1">
                <textarea name="description"
                          ...
                          placeholder="${t('codeExamples.form.descriptionPlaceholder')}">${codeExample?.description || ''}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <label for="language" class="block text-sm font-medium text-white">
                  ${t('codeExamples.language')} <span class="text-red-400">*</span>
                </label>
                <div class="mt-1">
              ...
              <!-- Category -->
              <div>
                <label for="category" class="block text-sm font-medium text-white">${t('codeExamples.category')}</label>
                <div class="mt-1">
                  <input type="text"
                         name="category"
                         id="category"
                         ...
                         maxlength="50"
                         class="block w-full rounded-md border-0 bg-gray-700 py-1.5 text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm sm:leading-6"
                         placeholder="${t('codeExamples.form.categoryPlaceholder')}">
                </div>
                ${errors?.category ? `
                  <div class="mt-1">
              ...
              <!-- Tags -->
              <div>
                <label for="tags" class="block text-sm font-medium text-white">${t('codeExamples.tags')}</label>
                <div class="mt-1">
                  <input type="text"
                         name="tags"
                         id="tags"
                         ...
                         maxlength="200"
                         class="block w-full rounded-md border-0 bg-gray-700 py-1.5 text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm sm:leading-6"
                         placeholder="${t('codeExamples.form.tagsPlaceholder')}">
          ...
          <!-- Code Section -->
          <div>
            <h2 class="text-lg font-medium text-white mb-4">${t('codeExamples.form.codeSnippet')}</h2>

            <!-- Code Editor -->
            <div class="mb-4">
              <label for="code" class="block text-sm font-medium text-white">
                ${t('codeExamples.code')} <span class="text-red-400">*</span>
              </label>
              <div class="mt-1">
                <textarea name="code"
                          id="code"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Published Status -->
            <div>
              <label class="block text-sm font-medium text-white">${t('codeExamples.status')}</label>
              <div class="mt-2 space-y-2">
                <div class="flex items-center">
                  ...
                  <label for="published" class="ml-2 block text-sm text-white">
                    ${t('codeExamples.published')} <span class="text-gray-300">(${t('codeExamples.form.publishedHelp')})</span>
                  </label>
                </div>
                <div class="flex items-center">
                  <input id="draft"
            ...
            <!-- Sort Order -->
            <div>
              <label for="sortOrder" class="block text-sm font-medium text-white">${t('codeExamples.order')}</label>
              <div class="mt-1">
                <input type="number"
                       name="sortOrder"
                       id="sortOrder"
                       ...
                       step="1"
                       class="block w-full rounded-md border-0 bg-gray-700 py-1.5 text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm sm:leading-6">
                <p class="mt-1 text-sm text-gray-300">${t('codeExamples.form.sortOrderHelp')}</p>
              </div>
              ${errors?.sortOrder ? `
                <div class="mt-1">
