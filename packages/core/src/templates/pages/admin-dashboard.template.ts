...
export function renderDashboardPage(data: DashboardPageData, t: TranslateFn): string {
  const pageContent = `
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl/8 font-semibold text-zinc-950 dark:text-white sm:text-xl/8">${t('dashboard.title')}</h1>
        <p class="mt-2 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('dashboard.welcome')}</p>
      </div>
      <div class="mt-4 sm:mt-0 flex items-center gap-x-3">
        <a href="/docs/getting-started" target="_blank" class="inline-flex items-center justify-center gap-x-1.5 rounded-lg bg-lime-600 dark:bg-lime-700 px-3.5 py-2.5 text-sm font-semibold text-white hover:bg-lime-700 dark:hover:bg-lime-600 transition-colors shadow-sm">
          ...
          ${t('dashboard.actions.developerDocs')}
        </a>
    ...
    <script>
      function refreshDashboard() {
        ...
        showNotification('${t('dashboard.actions.refresh')}', 'success');
      }
    </script>
  `;
...
export function renderStatsCards(stats: DashboardStats, t: TranslateFn): string {
  const cards = [
    {
    ...
    {
      title: t('dashboard.stats.contentItems'),
  ...
  return `
    <div>
      <h3 class="text-base font-semibold text-zinc-950 dark:text-white">${t('dashboard.stats.last30Days')}</h3>
          ...
          <div class="px-4 py-5 sm:p-6">
            ...
            <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
              ...
              <div class="inline-flex items-baseline rounded-full ${card.isPositive ? 'bg-lime-400/10 text-lime-600 dark:text-lime-400' : 'bg-pink-400/10 text-pink-600 dark:text-pink-400'} px-2.5 py-0.5 text-sm font-medium md:mt-2 lg:mt-0">
                ...
                <span class="sr-only">${card.isPositive ? t('dashboard.stats.increased') : t('dashboard.stats.decreased')} by</span>
                ${card.change}%
              </div>
            </dd>
          </div>
...
function renderAnalyticsChart(t: TranslateFn): string {
  return `
    <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10">
      <div class="border-b border-zinc-950/5 dark:border-white/10 px-6 py-6">
        <div class="flex flex-wrap items-start justify-between gap-3 sm:flex-nowrap">
          <div>
            <h3 class="text-base/7 font-semibold text-zinc-950 dark:text-white">${t('dashboard.analytics.title')}</h3>
            <p class="mt-1 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('dashboard.analytics.subtitle')}</p>
          </div>
          <div class="flex items-center gap-2">
            <div class="h-2 w-2 rounded-full bg-lime-500 animate-pulse"></div>
            <span class="text-xs text-zinc-500 dark:text-zinc-400">${t('dashboard.analytics.live')}</span>
          </div>
        </div>
        <div class="mt-4 flex items-baseline gap-2">
          <span id="current-rps" class="text-4xl font-bold text-cyan-500 dark:text-cyan-400">0</span>
          <span class="text-sm text-zinc-500 dark:text-zinc-400">${t('dashboard.analytics.requestsPerSecond')}</span>
        </div>
      </div>
    ...
    <script>
      // Initialize Chart.js for Real-time Requests with proper timing
      function initDashboardChart() {
        // Check if Chart.js is loaded
        if (typeof Chart === 'undefined') {
          console.warn('[Dashboard] Chart.js not loaded yet, retrying in 100ms...');
        ...
        if (!ctx) {
          console.error('[Dashboard] Canvas element #requestsChart not found');
          return;
        }

        console.log('[Dashboard] Initializing Chart.js...');
        ...
        window.addEventListener('htmx:afterRequest', function(event) {
          ...
          console.log('[Dashboard] HTMX request completed:', requestPath);

          // Match any path containing '/api/metrics'
          if (requestPath && requestPath.includes('/api/metrics')) {
            try {
              ...
              if (rpsElement) {
                ...
                console.log('[Dashboard] Updated display to:', metrics.requestsPerSecond.toFixed(2));
              } else {
                console.warn('[Dashboard] Element #current-rps not found');
              }
              ...
              console.error('[Dashboard] Error updating metrics:', e, e.stack);
            }
          }
        });
      }
...
export function renderRecentActivity(activities?: ActivityItem[], t?: TranslateFn): string {
  // Provide fallback function if t is not provided
  ...
  const getRelativeTime = (timestamp: string): string => {
    ...
    if (diffMins < 1) return translate('dashboard.recentActivity.justNow')
    if (diffMins < 60) return translate('dashboard.recentActivity.minutesAgo', { count: diffMins.toString(), s: diffMins > 1 ? 's' : '' })
    if (diffHours < 24) return translate('dashboard.recentActivity.hoursAgo', { count: diffHours.toString(), s: diffHours > 1 ? 's' : '' })
    return translate('dashboard.recentActivity.daysAgo', { count: diffDays.toString(), s: diffDays > 1 ? 's' : '' })
  }
  ...
  if (formattedActivities.length === 0) {
    formattedActivities.push({
      ...
      description: translate('dashboard.recentActivity.noActivity'),
      user: 'System',
      time: '',
      initials: 'SY',
      bgColor: 'bg-gray-500/10 dark:bg-gray-400/10',
    ...
    <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10">
      <div class="border-b border-zinc-950/5 dark:border-white/10 px-6 py-6">
        <div class="flex items-center justify-between">
          <h3 class="text-base/7 font-semibold text-zinc-950 dark:text-white">${translate('dashboard.recentActivity.title')}</h3>
          <a href="/admin/activity-logs" class="text-xs/5 font-medium text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-300 transition-colors">
            ${translate('dashboard.recentActivity.viewAll')}
          </a>
        </div>
      </div>
...
function renderQuickActions(t: TranslateFn): string {
  const actions = [
    {
      title: t('dashboard.quickActions.createContent'),
      description: t('dashboard.quickActions.createContentDesc'),
    ...
    {
      title: t('dashboard.quickActions.uploadMedia'),
      description: t('dashboard.quickActions.uploadMediaDesc'),
      href: "/admin/media",
      icon: `<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return `
    <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10">
      <div class="border-b border-zinc-950/5 dark:border-white/10 px-6 py-6">
        <h3 class="text-base/7 font-semibold text-zinc-950 dark:text-white">${t('dashboard.quickActions.title')}</h3>
      </div>
...
function renderSystemStatus(t: TranslateFn): string {
  return `
    <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 overflow-hidden">
      <div class="border-b border-zinc-950/5 dark:border-white/10 px-6 py-6">
        <div class="flex items-center justify-between">
          <h3 class="text-base/7 font-semibold text-zinc-950 dark:text-white">${t('dashboard.systemStatus.title')}</h3>
          <div class="flex items-center gap-2">
            <div class="h-2 w-2 rounded-full bg-lime-500 animate-pulse"></div>
            <span class="text-xs text-zinc-500 dark:text-zinc-400">${t('dashboard.systemStatus.live')}</span>
          </div>
        </div>
      </div>
...
export function renderStorageUsage(databaseSizeBytes?: number, mediaSizeBytes?: number, t?: TranslateFn): string {
  // Provide fallback function if t is not provided
  const translate = t || ((key: string) => key)
  ...
  const storageItems = [
    {
      label: translate('dashboard.storage.database'),
    ...
    {
      label: translate('dashboard.storage.mediaFiles'),
      used: mediaUsedFormatted,
      total: "∞",
      percentage: 0,
      color: "bg-lime-500 dark:bg-lime-400",
      note: translate('dashboard.storage.storedInR2')
    },
    {
      label: translate('dashboard.storage.cache'),
      used: "N/A",
      total: "∞",
      percentage: 0,
      color: "bg-purple-500 dark:bg-purple-400",
      note: translate('dashboard.storage.unlimited')
    },
  ];
  ...
  return `
    <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10">
      <div class="border-b border-zinc-950/5 dark:border-white/10 px-6 py-6">
        <h3 class="text-base/7 font-semibold text-zinc-950 dark:text-white">${translate('dashboard.storage.title')}</h3>
      </div>
