...
export function createHelloWorldPlugin(): Plugin {
  ...
  helloWorldRoutes.use('*', i18nMiddleware())
    ...
    return c.html(html`
      ...
      <html lang="en" class="dark">
        <head>
          ...
          <title>Hello World - PatroCMS</title>
        ...
        <body class="bg-white dark:bg-zinc-950 text-zinc-950 dark:text-white min-h-screen">
          <div class="flex h-screen">
            ...
            <aside class="w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800">
              <div class="p-4">
                <h2 class="text-xl font-bold mb-4">PatroCMS</h2>
                <nav>
            ...
            <main class="flex-1 overflow-y-auto">
              <div class="p-8">
                <!-- Header -->
                <div class="mb-8">
                  <h1 class="text-3xl font-bold mb-2">${t('plugins.helloWorld.title')}</h1>
                  <p class="text-zinc-600 dark:text-zinc-400">${t('plugins.helloWorld.subtitle')}</p>
                </div>
                ...
                <div class="max-w-2xl">
                  <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                    <div class="p-8 text-center">
                      ...
                      <h2 class="text-4xl font-bold mb-4 bg-gradient-to-r from-blue-500 to-purple-600 bg-clip-text text-transparent">
                        ${t('plugins.helloWorld.greeting')}
                      </h2>
                      <p class="text-lg text-zinc-600 dark:text-zinc-400 mb-6">
                        ${t('plugins.helloWorld.description')}
                      </p>
                      <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 text-left">
                        <h3 class="font-semibold mb-2">${t('plugins.helloWorld.pluginInfo')}:</h3>
                        <ul class="text-sm text-zinc-600 dark:text-zinc-400 space-y-1">
                          <li><strong>${t('plugins.helloWorld.pluginId')}:</strong> hello-world</li>
                          <li><strong>${t('plugins.helloWorld.version')}:</strong> 1.0.0-beta.1</li>
                          <li><strong>${t('plugins.helloWorld.user')}:</strong> ${user?.email || t('plugins.helloWorld.notLoggedIn')}</li>
                          <li><strong>${t('plugins.helloWorld.role')}:</strong> ${user?.role || 'N/A'}</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="mt-6 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                    <h3 class="font-semibold text-blue-900 dark:text-blue-100 mb-2">
                      📚 ${t('plugins.helloWorld.howItWorks')}
                    </h3>
                    <ul class="text-sm text-blue-800 dark:text-blue-200 space-y-2">
                      <li>✓ ${t('plugins.helloWorld.featureDiscovery')}</li>
                      <li>✓ ${t('plugins.helloWorld.featureRegistry')}</li>
                      <li>✓ ${t('plugins.helloWorld.featureNavigation')}</li>
                      <li>✓ ${t('plugins.helloWorld.featureRouteHandler')}</li>
                      <li>✓ ${t('plugins.helloWorld.featureIntegration')}</li>
                    </ul>
                  </div>
                </div>
              </div>
