  ...
  return [
    {
      ...
      displayName: "Text",
      description:
        "Single-line text input for short strings, names, titles, etc.",
      category: "Basic",
      ...
      validations: [
        ...
        },
        {
          ...
          example:
            'if (!pattern.test(value)) throw new Error("Invalid format")',
        },
      ],
      examples: [
        {
        ...
        },
        {
          ...
          renderedOutput:
            '<input type="text" pattern="^[a-z0-9-]+$" placeholder="my-url-slug">',
        },
      ],
      useCases: [
      ...
      validations: [
        {
          ...
          example:
            'if (stripHtml(value).trim() === "") throw new Error("Content required")',
        },
        {
      ...
      examples: [
        {
          ...
          sampleValue:
            "<h2>Introduction</h2><p>This is <strong>rich</strong> content...</p>",
          renderedOutput: "Rich text editor with full toolbar",
        },
        {
          title: "Simple Comment Editor",
          configuration: { height: 150, toolbar: "simple" },
          sampleValue:
            "<p>A simple comment with <em>basic</em> formatting.</p>",
          renderedOutput: "Rich text editor with limited toolbar",
        },
      ],
      useCases: [
    ...
    },
    {
      ...
      validations: [
        ...
        },
        {
          ...
          example:
            'if ((value - min) % step !== 0) throw new Error("Invalid step")',
        },
      ],
      examples: [
        {
      ...
      examples: [
        {
          ...
          sampleValue: true,
          renderedOutput:
            '<input type="checkbox" checked> <label>Published</label>',
        },
        {
          ...
          sampleValue: false,
          renderedOutput:
            '<input type="checkbox"> <label>I agree to the terms of service</label>',
        },
      ],
      useCases: [
      ...
      validations: [
        {
          ...
          example:
            'if (!value || !isValidDate(value)) throw new Error("Date required")',
        },
        {
          name: "Date Range",
          ...
          example:
            'if (date < minDate || date > maxDate) throw new Error("Date out of range")',
        },
        {
          ...
          example:
            'if (!/\\d{4}-\\d{2}-\\d{2}/.test(value)) throw new Error("Invalid format")',
        },
      ],
      examples: [
        {
        ...
        },
        {
          ...
          renderedOutput:
            '<input type="date" min="2024-01-01" value="2024-07-10">',
        },
      ],
      useCases: [
      ...
      validations: [
        {
          ...
          example:
            'if (!value || (Array.isArray(value) && value.length === 0)) throw new Error("Selection required")',
        },
        {
          name: "Valid Option",
          ...
          example:
            'if (!allowedValues.includes(value)) throw new Error("Invalid selection")',
        },
        {
          ...
          example:
            'if (values.length > maxSelections) throw new Error("Too many selections")',
        },
      ],
      examples: [
        {
          ...
          renderedOutput:
            '<select><option value="draft">Draft</option><option value="published" selected>Published</option></select>',
        },
        {
          ...
          renderedOutput:
            '<select multiple><option value="tech" selected>Technology</option><option value="design" selected>Design</option></select>',
        },
      ],
      useCases: [
    ...
    },
    {
      ...
      displayName: "Media",
      description:
        "File upload and selection for images, videos, and documents",
      category: "Media",
      icon: `<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      validations: [
        {
          ...
          example:
            'if (!value || !isValidUrl(value)) throw new Error("Media file required")',
        },
        {
          name: "File Type",
  ...
  const pageContent = `
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div class="backdrop-blur-xl bg-white/10 rounded-xl border border-white/20 shadow-2xl p-6 mb-6">
        ...
        <div class="flex flex-wrap gap-2" id="category-filters">
          ${getUniqueCategories(data.fieldTypes)
            .map(
              (category) => `
            <button 
      ...
      <div class="grid gap-6" id="field-types-container">
        ${data.fieldTypes
          .map((fieldType) => renderFieldTypeCard(fieldType))
          .join("")}
      </div>
    </div>
    ...
    <script>
      ...
      function exportFieldTypes() {
        ...
        a.href = url;
        a.download = 'patro-field-types-reference.json';
        a.click();
...
function renderFieldTypeCard(fieldType: FieldTypeDefinition): string {
  return `
    <div class="field-type-card backdrop-blur-xl bg-white/5 rounded-xl border border-white/10 p-6" data-category="${
      fieldType.category
    }">
      <!-- Header -->
      <div class="flex items-start justify-between mb-4">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-sm text-gray-400">${fieldType.category} • ${
    fieldType.dataType
  }</p>
          </div>
        </div>
        <div class="flex items-center space-x-2">
          <span class="px-2 py-1 bg-gray-800 text-xs text-gray-300 rounded-md font-mono">${
            fieldType.name
          }</span>
          ${
            fieldType.requiresScript
              ? '<span class="px-2 py-1 bg-orange-500/20 text-orange-300 text-xs rounded-md">Script</span>'
              : ""
          }
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="border border-white/10 rounded-lg">
          ...
          >
            <span class="font-medium text-white">Configuration Options (${
              fieldType.supportedOptions.length
            })</span>
            <svg id="${
              fieldType.name
            }-options-icon" class="w-4 h-4 text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <div id="${
            fieldType.name
          }-options" style="display: none;" class="px-3 pb-3">
            <div class="space-y-3">
              ${fieldType.supportedOptions
                .map(
                  (option) => `
                <div class="bg-gray-800/30 rounded-lg p-3">
                  <div class="flex items-start justify-between mb-2">
                    <div>
                      <code class="text-blue-400 font-medium">${
                        option.name
                      }</code>
                      ...
                      ${
                        option.required
                          ? '<span class="ml-1 text-red-400 text-xs">required</span>'
                          : ""
                      }
                    </div>
                    ${
                      option.defaultValue !== undefined
                        ? `<span class="text-xs text-gray-400">default: <code class="text-green-400">${JSON.stringify(
                            option.defaultValue
                          )}</code></span>`
                        : ""
                    }
                  ...
                  ${
                    ...
                    <div class="text-xs text-gray-400">
                      Examples: ${option.examples
                        .map(
                          (ex) => `<code class="text-purple-400">${ex}</code>`
                        )
                        .join(", ")}
                    </div>
                  `
        ...
        <div class="border border-white/10 rounded-lg">
          ...
          >
            <span class="font-medium text-white">Validation Rules (${
              fieldType.validations.length
            })</span>
            <svg id="${
              fieldType.name
            }-validations-icon" class="w-4 h-4 text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <div id="${
            fieldType.name
          }-validations" style="display: none;" class="px-3 pb-3">
            <div class="space-y-3">
              ${fieldType.validations
                .map(
                  (validation) => `
                <div class="bg-gray-800/30 rounded-lg p-3">
                  <h5 class="text-yellow-400 font-medium mb-1">${
                    validation.name
                  }</h5>
                  <p class="text-sm text-gray-300 mb-2">${
                    validation.description
                  }</p>
                  <div class="text-xs text-gray-400 mb-2">Implementation: ${
                  ...
                  <div class="bg-gray-900/50 rounded p-2">
                    ...
                    <button 
                      onclick="copyExample('${validation.example.replace(
                        /'/g,
                        "\\'"
                      )}')"
        ...
        <div class="border border-white/10 rounded-lg">
          ...
          >
            <span class="font-medium text-white">Usage Examples (${
              fieldType.examples.length
            })</span>
            <svg id="${
              fieldType.name
            }-examples-icon" class="w-4 h-4 text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <div id="${
            fieldType.name
          }-examples" style="display: none;" class="px-3 pb-3">
            <div class="space-y-3">
              ${fieldType.examples
                .map(
                  (example) => `
                <div class="bg-gray-800/30 rounded-lg p-3">
                  <h5 class="text-purple-400 font-medium mb-2">${
                    example.title
                  }</h5>
                  <div class="grid gap-2 text-xs">
                    <div>
                      ...
                      <div class="bg-gray-900/50 rounded p-2 mt-1">
                        <code class="text-blue-400">${JSON.stringify(
                    ...
                    <div>
                      ...
                      <div class="bg-gray-900/50 rounded p-2 mt-1">
                        <code class="text-green-400">${JSON.stringify(
                          example.sampleValue
                        )}</code>
                      </div>
                    </div>
        ...
        <div class="border border-white/10 rounded-lg">
          ...
          >
            ...
            <svg id="${
              fieldType.name
            }-usecases-icon" class="w-4 h-4 text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <div id="${
            fieldType.name
          }-usecases" style="display: none;" class="px-3 pb-3">
            <ul class="space-y-1">
              ${fieldType.useCases
                .map(
                  (useCase) => `
