...
export function renderPluginSettingsPage(data: PluginSettingsPageData, t: TranslateFn): string {
    ...
    <script>
      ...
      async function saveSettings() {
        ...
        if (isAuthPlugin) {
          ...
          const isAiTranslator = '${plugin.id}' === 'ai-translator';
          ...
          // Special handling for AI Translator targetLanguages checkboxes
          if (isAiTranslator) {
            const targetLanguages = [];
            form.querySelectorAll('input[name^="setting_targetLanguages_"]:checked').forEach(cb => {
              targetLanguages.push(cb.value);
            });
            settings.targetLanguages = targetLanguages;
            
            // Convert translatableFields from string to array
            if (settings.translatableFields && typeof settings.translatableFields === 'string') {
              settings.translatableFields = settings.translatableFields
                .split(',')
  ...
  const layoutData: AdminLayoutData = {
    title: t('plugins.settings.title', { name: plugin.displayName }) || `${plugin.displayName} Settings`,
    pageTitle: t('plugins.settings.title', { name: plugin.displayName }) || `${plugin.displayName} Settings`,
  ...
  // Pass translate function so sidebar/navigation uses correct locale
...
function renderSettingsTab(plugin: any): string {
  ...
  const isAiTranslatorPlugin = plugin.id === 'ai-translator' || plugin.name === 'ai-translator'
  ...
  return `
    ...
    <div class="backdrop-blur-md bg-black/20 rounded-xl border border-white/10 shadow-xl p-6">
      ...
      ` : isAiTranslatorPlugin ? `
        <h2 class="text-xl font-semibold text-white mb-4">AI Translator Settings</h2>
        <p class="text-gray-400 mb-6">Configure automatic translation settings and target languages.</p>
      ` : `
      ...
      <form id="settings-form" class="space-y-6">
        ${isAuthPlugin && Object.keys(settings).length > 0
          ...
          : isAiTranslatorPlugin && Object.keys(settings).length > 0
            ? renderAiTranslatorSettingsForm(settings)
      ...
      return `
        <div>
          ...
          <textarea
            name="${fieldId}"
            id="${fieldId}"
            class="backdrop-blur-sm bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white placeholder-gray-300 focus:border-blue-400 focus:outline-none transition-colors w-full h-32"
...
function renderAiTranslatorSettingsForm(settings: any): string {
  const SUPPORTED_LANGUAGES = ['cs', 'en', 'de', 'fr', 'es', 'it', 'pl']
  ...
  return `
    ...
    <div class="flex items-center justify-between">
      <div>
        <label for="setting_enabled" class="text-sm font-medium text-gray-300">Enable Auto Translation</label>
        <p class="text-xs text-gray-400">Enable or disable automatic translation</p>
      </div>
      <label class="relative inline-flex items-center cursor-pointer">
    ...
    <div>
      <label for="setting_defaultSourceLanguage" class="block text-sm font-medium text-gray-300 mb-2">Default Source Language</label>
      <select
        name="setting_defaultSourceLanguage"
        id="setting_defaultSourceLanguage"
      ...
      >
        ${SUPPORTED_LANGUAGES.map(lang => `
          <option value="${lang}" ${settings.defaultSourceLanguage === lang ? 'selected' : ''}>
    ...
    <div>
      <label class="block text-sm font-medium text-gray-300 mb-2">Target Languages</label>
      <div class="grid grid-cols-2 gap-2">
        ${SUPPORTED_LANGUAGES.map(lang => `
          <div class="flex items-center">
            <input
              type="checkbox"
              id="setting_targetLanguages_${lang}"
              name="setting_targetLanguages_${lang}"
              value="${lang}"
              ${Array.isArray(settings.targetLanguages) && settings.targetLanguages.includes(lang) ? 'checked' : ''}
              class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <label for="setting_targetLanguages_${lang}" class="ml-2 text-sm text-gray-300">
              ${LANGUAGE_NAMES[lang]} (${lang})
            </label>
          </div>
        `).join('')}
    ...
    <div class="space-y-2">
      <label class="block text-sm font-medium text-gray-300 mb-2">Translation Triggers</label>
      <div class="flex items-center">
        <input
          type="checkbox"
          id="setting_translateOnCreate"
          name="setting_translateOnCreate"
          ${settings.translateOnCreate ? 'checked' : ''}
          class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
        />
        <label for="setting_translateOnCreate" class="ml-2 text-sm text-gray-300">
          Translate on Create
        </label>
      </div>
      <div class="flex items-center">
        <input
          type="checkbox"
          id="setting_translateOnUpdate"
          name="setting_translateOnUpdate"
          ${settings.translateOnUpdate ? 'checked' : ''}
          class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
        />
        <label for="setting_translateOnUpdate" class="ml-2 text-sm text-gray-300">
          Translate on Update
        </label>
      </div>
    </div>
    ...
    <!-- Translatable Fields -->
    <div>
      <label for="setting_translatableFields" class="block text-sm font-medium text-gray-300 mb-2">Translatable Fields</label>
      <input
        type="text"
        id="setting_translatableFields"
        name="setting_translatableFields"
        value="${Array.isArray(settings.translatableFields) ? settings.translatableFields.join(', ') : settings.translatableFields || ''}"
        class="backdrop-blur-sm bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white placeholder-gray-300 focus:border-blue-400 focus:outline-none transition-colors w-full"
        placeholder="title, content, excerpt, description"
      />
      <p class="text-xs text-gray-400 mt-1">
        Comma-separated list of fields to translate
      </p>
    </div>

    <!-- AI Model (Dropdown) -->
    <div>
      <label for="setting_aiModel" class="block text-sm font-medium text-gray-300 mb-2">AI Model</label>
      <select
        name="setting_aiModel"
        id="setting_aiModel"
        class="backdrop-blur-sm bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white focus:border-blue-400 focus:outline-none transition-colors w-full"
      >
        <option value="@cf/meta/m2m100-1.2b" ${settings.aiModel === '@cf/meta/m2m100-1.2b' ? 'selected' : ''}>
          Meta M2M100 1.2B (@cf/meta/m2m100-1.2b)
        </option>
      </select>
      <p class="text-xs text-gray-400 mt-1">
        Select the AI model for content translation
      </p>
    </div>
  `
}
