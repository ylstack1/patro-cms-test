...
// Helper to translate common field labels
function translateFieldLabel(fieldName: string, defaultLabel: string, t?: TranslateFn): string {
...
export function renderDynamicField(
  ...
  if (field.field_type === "quill" && !pluginStatuses.quillEnabled) {
    ...
    fallbackWarning =
      "⚠️ Quill Editor plugin is inactive. Using textarea fallback.";
  } else if (
    field.field_type === "mdxeditor" &&
    !pluginStatuses.mdxeditorEnabled
  ) {
    fallbackToTextarea = true;
    fallbackWarning =
      "⚠️ MDXEditor plugin is inactive. Using textarea fallback.";
  }
  ...
  if (fallbackToTextarea) {
    return `
      <div>
        ${
          fallbackWarning
            ? `<div class="mb-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/20 text-amber-900 dark:text-amber-200 text-xs rounded-lg border border-amber-200 dark:border-amber-800">${fallbackWarning}</div>`
            : ""
        }
        <textarea
          id="${fieldId}"
  ...
  switch (field.field_type) {
    ...
    case "richtext":
      // Decision logic for richtext field type:
      // 1. If Quill is enabled, use Quill (render quill structure)
      // 2. If MDX is enabled (and Quill is not), use MDX (render richtext-container)
      // 3. Fallback to plain textarea

      if (pluginStatuses.quillEnabled) {
        // Render as Quill
        console.log('🔍 [RENDER] Rendering richtext as Quill:', fieldName);
        fieldHTML = `
        ...
        `;
      } else if (pluginStatuses.mdxeditorEnabled) {
        // Render as MDX (EasyMDE)
        fieldHTML = `
          <div class="richtext-container" data-height="${
            opts.height || 300
          }" data-toolbar="${opts.toolbar || "full"}">
            <textarea
              id="${fieldId}"
              name="${fieldName}"
              class="${baseClasses} ${errorClasses} min-h-[${
          opts.height || 300
        }px]"
              ${required}
              ${disabled ? "disabled" : ""}
    ...
    case "quill":
      // Explicit Quill WYSIWYG Editor
      console.log('🔍 [RENDER] Rendering explicit Quill field:', fieldName);
      fieldHTML = `
        <div class="quill-editor-container" data-field-id="${fieldId}">
          <!-- Quill Editor Container -->
          <div
          ...
          ${options
            .map((option: any) => {
              ...
              return `<option value="${escapeHtml(
                optionValue
              )}" ${selected}>${escapeHtml(optionLabel)}</option>`;
            })
            .join("")}
        </select>
        ${
      ...
      fieldHTML = `
        <div class="media-field-container">
          ...
          <div class="media-preview ${
            ...
            ${
              value
                ? `<img src="${value}" alt="Selected media" class="w-32 h-32 object-cover rounded-lg border border-white/20">`
                : ""
            }
          </div>
  ...
  const translatedLabel = translateFieldLabel(field.field_name, field.field_label, options.t);
  ...
  return `
    <div class="form-group">
      <label for="${fieldId}" class="block text-sm/6 font-medium text-zinc-950 dark:text-white mb-2">
        ...
        ${
          field.is_required
            ? '<span class="text-pink-600 dark:text-pink-400 ml-1">*</span>'
            : ""
        }
      </label>
      ${fieldHTML}
...
export function renderFieldGroup(
  ...
  return `
    <div class="field-group rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 mb-6">
      <div class="field-group-header border-b border-zinc-950/5 dark:border-white/10 px-6 py-4 ${
        collapsible ? "cursor-pointer" : ""
      }" ${collapsible ? `onclick="toggleFieldGroup('${groupId}')"` : ""}>
      ...
      <div id="${groupId}-content" class="field-group-content px-6 py-6 space-y-6 ${
