...
import { renderAdminLayoutCatalyst, AdminLayoutCatalystData } from "./admin-layout-catalyst.template";
import type { TranslateFn } from "../../services/i18n";
import { createDummyTranslate } from "../../utils/dummy-translate";
...
export function renderAdminLayout(data: AdminLayoutData, t?: TranslateFn): string {
  // Use the new Catalyst layout with proper i18n support
  // If no translate function provided, use dummy translate as fallback for backward compatibility
  const translate = t || createDummyTranslate();
  return renderAdminLayoutCatalyst(data, translate);
}
...
<body class="bg-gradient-to-br from-slate-900 via-gray-900 to-black min-h-screen text-gray-1">
  ...
  <div class="relative z-10 min-h-screen">
    <!-- Header -->
    ${renderTopBar(data.pageTitle || "Dashboard", data.user, data.logoUrl)}
...
function renderTopBar(pageTitle: string, user?: any, logoUrl?: string): string {
  return `
    <header class="backdrop-blur-md bg-white/10 border-b border-white/20 shadow-lg relative z-[9998]">
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
          <div class="flex items-center space-x-4">
            ${renderLogo({ size: "md", showText: true, variant: "white", logoUrl })}
          </div>
