...
export class SeedDataService {
  ...
  // Create 20 content items across different types
  async createContent(): Promise<number> {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (collection.name === 'blog_posts' || collection.name.includes('blog')) {
        ...
        contentData = {
          content: this.blogContent[Math.floor(Math.random() * this.blogContent.length)] || 'Blog content here',
        ...
        contentData = {
          content: 'This is a standard page with important information about our services and policies.',
          template: 'default',
  ...
  async clearSeedData(): Promise<void> {
    ...
    const deleteUsersStmt = this.db.prepare(
      "DELETE FROM users WHERE email != 'admin@patro.io'"
    )
