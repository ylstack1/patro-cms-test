  ...
  private async deliverWebhook(
    ...
    try {
      ...
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'PatroCMS-Webhooks/1.0',
        'X-PatroCMS-Event': eventType,
        'X-PatroCMS-Delivery': deliveryId,
        'X-PatroCMS-Timestamp': timestamp
      }

      // Add signature if secret is provided
      if (webhook.secret) {
        const signature = await this.createSignature(JSON.stringify(webhookPayload), webhook.secret)
        headers['X-PatroCMS-Signature'] = signature
      }
