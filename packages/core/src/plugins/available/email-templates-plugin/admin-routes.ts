...
import { requireAuth, requireRole } from '@patro-io/cms';
...
// Create theme (POST)
const createThemeSchema = Schema.Struct({
  name: Schema.String.pipe(Schema.minLength(1)),
  description: Schema.optional(Schema.String),
  htmlTemplate: Schema.String.pipe(Schema.minLength(1)),
  cssStyles: Schema.String.pipe(Schema.minLength(1)),
  isDefault: Schema.optional(Schema.String)
})

emailRoutes.post('/themes', zValidator('form', createThemeSchema), async (c) => {
  const user = c.get('user');
  const data = c.req.valid('form') as any;
  const emailService = createEmailManagementService(c.env, user.userId);
...
const createTemplateSchema = Schema.Struct({
  ...
  themeId: Schema.String.pipe(Schema.minLength(1)),
  markdownContent: Schema.String.pipe(Schema.minLength(1)),
  isActive: Schema.optional(Schema.String)
})

emailRoutes.post('/templates', zValidator('form', createTemplateSchema), async (c) => {
  const user = c.get('user');
  const data = c.req.valid('form') as any;
  const emailService = createEmailManagementService(c.env, user.userId);
