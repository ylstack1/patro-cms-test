...
export interface AdminLayoutCatalystData {
  ...
  /** Logo URL from appearance settings */
  logoUrl?: string;
}
...
<head>
  ...
  <title>${data.title} - PatroCMS Admin</title>
  ...
  <div class="relative isolate flex min-h-svh w-full max-lg:flex-col lg:bg-zinc-100 dark:lg:bg-zinc-950">
    ...
    <main class="flex flex-1 flex-col pb-2 lg:min-w-0 lg:pr-2 lg:pl-64">
      ...
      <header class="flex items-center px-4 py-2.5 lg:hidden border-b border-zinc-950/5 dark:border-white/5">
        <button onclick="openMobileSidebar()" class="relative flex items-center justify-center rounded-lg p-2 text-zinc-950 hover:bg-zinc-950/5 dark:text-white dark:hover:bg-white/5" aria-label="${t('navigation.openNavigation')}">
        ...
        <div class="ml-4 flex-1">
          <a href="/admin" class="flex items-center gap-2">
            <h1 class="text-xl font-semibold tracking-tight text-white">PatroCMS</h1>
            ${data.version ? `<span class="text-xs text-zinc-400">v${data.version}</span>` : ''}
          </a>
        </div>
      </header>
    ...
    <div class="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between flex-wrap">
        <div class="flex items-center flex-1">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-white" id="migration-message">
            </p>
            <p class="text-xs text-amber-100 dark:text-amber-200 mt-1">
              ${t('migrations.runCommand')} <code class="bg-amber-700 dark:bg-amber-800 px-2 py-0.5 rounded font-mono text-white">wrangler d1 migrations apply DB --local</code>
            </p>
          </div>
        </div>
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data && data.data.pendingMigrations > 0) {
            const banner = document.getElementById('migration-banner');
            const messageElement = document.getElementById('migration-message');
            if (banner && messageElement) {
              messageElement.textContent = \`\${data.data.pendingMigrations} ${t('migrations.pendingDetected').replace('{count}', '')}\`;
...
function renderCatalystSidebar(
  currentPath: string = "",
  user: any | undefined,
  dynamicMenuItems: Array<{ label: string; path: string; icon: string }> | undefined,
  ...
  const closeButton = isMobile
    ...
    <div class="-mb-3 px-4 pt-3">
      <button onclick="closeMobileSidebar()" class="relative flex w-full items-center gap-3 rounded-lg p-2 text-left text-base/6 font-medium text-zinc-950 hover:bg-zinc-950/5 dark:text-white dark:hover:bg-white/5 sm:text-sm/5" aria-label="${t('navigation.closeNavigation')}">
        <svg class="h-5 w-5 shrink-0 fill-zinc-500 dark:fill-zinc-400" viewBox="0 0 20 20">
          <path d="M6.28 5.22a.75.75 0 0 0-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 1 0 1.06 1.06L10 11.06l3.72 3.72a.75.75 0 1 0 1.06-1.06L11.06 10l3.72-3.72a.75.75 0 0 0-1.06-1.06L10 8.94 6.28 5.22Z" />
        </svg>
        <span>${t('navigation.closeMenu')}</span>
      </button>
    </div>
  `
    : "";
  ...
  return `
    <nav class="flex h-full min-h-0 flex-col bg-white shadow-sm ring-1 ring-zinc-950/5 dark:bg-zinc-900 dark:ring-white/10 ${
      ...
      <div class="flex flex-col border-b border-zinc-950/5 p-4 dark:border-white/5">
        <a href="/admin">
          <h1 class="text-3xl font-semibold tracking-tight text-white">PatroCMS</h1>
          ${version ? `<span class="text-xs text-zinc-400 mt-1">v${version}</span>` : ''}
        </a>
      </div>
