...
export function renderCollectionsListPage(data: CollectionsListPageData, t: TranslateFn): string {
  const tableData: any = {
    ...
    columns: [
      {
        ...
        render: (_value: any, collection: any) => `
            <div class="flex items-center gap-2 ml-2">
                ...
                ${collection.managed ? `
                  <span class="inline-flex items-center rounded-md bg-purple-50 dark:bg-purple-500/10 px-2 py-0.5 text-xs font-medium text-purple-700 dark:text-purple-300 ring-1 ring-inset ring-purple-700/10 dark:ring-purple-400/20" title="${t('collections.managed')}">
          ...
          return `
            <div class="flex items-center">
              <span class="inline-flex items-center rounded-md bg-pink-50 dark:bg-pink-500/10 px-2.5 py-1 text-sm font-medium text-pink-700 dark:text-pink-300 ring-1 ring-inset ring-pink-700/10 dark:ring-pink-400/20">
                ${count} ${count === 1 ? t('collections.field') : t('collections.fields')}
              </span>
            </div>
          `
        }
          ...
          if (collection.managed) {
            return `
              <div class="flex items-center gap-1.5">
                ...
                <span class="text-sm text-zinc-700 dark:text-zinc-300">${t('collections.code')}</span>
              </div>
            `
          } else {
            return `
              <div class="flex items-center gap-1.5">
                ...
                <span class="text-sm text-zinc-700 dark:text-zinc-300">${t('collections.database')}</span>
              </div>
            `
          }
        }
    ...
    emptyMessage: t('collections.messages.noCollections')
  }
  ...
  const pageContent = `
    <div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 class="text-2xl/8 font-semibold text-zinc-950 dark:text-white sm:text-xl/8">${t('collections.title')}</h1>
          <p class="mt-2 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('collections.subtitle')}</p>
        </div>
          ...
          <div class="px-6 py-5">
            <div class="flex items-center justify-between">
                ...
                <form onsubmit="performSearch(event)" class="flex items-center space-x-2">
                  <div class="relative group">
                    <input
                      ...
                      type="text"
                      placeholder="${t('collections.actions.searchPlaceholder')}"
                      value="${data.search || ''}"
              ...
              <div class="flex items-center gap-x-3">
                <span class="text-sm/6 font-medium text-zinc-700 dark:text-zinc-300 px-3 py-1.5 rounded-full bg-white/60 dark:bg-zinc-800/60 backdrop-blur-sm">${data.collections.length === 1 ? t('collections.count.singular', { count: data.collections.length }) : t('collections.count.plural', { count: data.collections.length })}</span>
                <button
      ...
      ${data.collections.length === 0 ? `
        <div class="rounded-lg bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-950/5 dark:ring-white/10 p-12 text-center">
          ...
          <h3 class="mt-4 text-base/7 font-semibold text-zinc-950 dark:text-white">${t('collections.messages.noCollections')}</h3>
          <p class="mt-2 text-sm/6 text-zinc-500 dark:text-zinc-400">${t('collections.messages.getStarted')}</p>
          <div class="mt-6">
            <a href="/admin/collections/new" class="inline-flex items-center justify-center rounded-lg bg-zinc-950 dark:bg-white px-3.5 py-2.5 text-sm font-semibold text-white dark:text-zinc-950 hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors shadow-sm">
              <svg class="-ml-0.5 mr-1.5 h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
