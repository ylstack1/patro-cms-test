...
import { PluginConfig } from '@patro-io/cms'
...
const PluginConfigSchema = Schema.Struct({
  ...
  environments: Schema.optional(Schema.Array(Schema.Literal('development', 'staging', 'production'))),
...
export class PluginConfigManager {
  ...
  setConfig(pluginName: string, config: PluginConfig): void {
    ...
    const validation = Schema.decodeUnknownEither(PluginConfigSchema)(config)
    if (validation._tag === 'Left') {
      throw new Error(`Invalid plugin configuration for ${pluginName}: ${validation.left.message}`)
    }
  ...
  validateConfig(_pluginName: string, config: PluginConfig): { valid: boolean; errors: string[] } {
    ...
    const errors = [validation.left.message]
   ...
   */
  getConfigSchema(_pluginName: string): Schema.Schema.Any | undefined {
